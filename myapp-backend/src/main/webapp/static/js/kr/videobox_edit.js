...
$(function(){
    ...
    $("#imgUpload").fileinput({
        ...
        showPreview: true,
        showCaption: false,
        allowedFileExtensions: ["jpg", "png","mp3","mp4","obj","mtl"],
        ...
        if (type=="image/jpeg"||type=="image/png"||type=="audio/mp3"||type=="image/gif"||type=="video/mp4"||type==""){
...
function upload_img(){
    $("#media_icon").modal('hide');
    if(click_type == 'videobox_img'){
        $.post("/edit/videobox",{
            act:'videobox_img_add',
            rid:rid,
            key:tokenObj.key,
        },function(data){
            if(data.status==1){
         ...
         $.post("/edit/videobox",{
            ...
            if(data.status==1){
                if(click_type=='mp4'||click_type=='mp3'){
...
function preview_videobox(id){
    window.open("/720play?id="+id+"");
}
