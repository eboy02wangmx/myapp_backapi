...
var openMediaResObj;
var mapModalEl;
var hotSpotModalTitle = ["シーン切替スポット追加","添加超链接热点","添加图册","添加文本热点","添加音乐","添加图文热点","添加分组","添加环物热点","添加视频热点","添加红包"];
...
$(function () {
    ...
    imgtext_editor = KindEditor.create('#imgtext_editor', {
        basePath: '/myapp-backend/static/kindeditor/',
    ...
    imgtext_editor.clickToolbar('image', function() {
        ...
        $("#music").html("");
        //查询图片媒体资源
        var sb = _U.getSubmit("/myapp-backend/member/mediares", null, "ajax", false);
    ...
    $("#imgUpload").fileinput({
        ...
        if (mediaType == '1') {
            ...
            msg = 'アップロード完了';
            addImageMediaRes(data.response,true);
        }
...
function renameImg(pk_img_main, new_img_name) {
    var result = false;
    var sb = _U.getSubmit("/myapp-backend/edit/pic", null, "ajax", true);
        ...
        if(this.name == hotspotName){
            ...
            switch(hotspotidx){
                ...
                case 1:
					...
					var isNewWindow = $('#hot_name input[type=checkbox]:eq(0)').is(':checked');
            ...
            switch(hotspotidx){
                case 0:
                    ...
                    break;
                case 1:
                    krpano.call('addLinkHotSpot("'+self.imgPath+'","'+ (self.name) +'",'+html_encode(self.hotspotTitle)+','+(self.ath)+','+(self.atv)+','+self.isDynamic+',true,true,'+html_encode(self.link)+','+self.isShowSpotName+','+param.isNewWindow+')');
                    break;
                case 2:
					if(param.iconType=="polygon"){
                        setPolygonParams(self,hotSpotDataKey[hotspotidx]);
...
function resetHotSpotCustomIcon(){
    $('#icon_style .media_icons img').attr('src',"/myapp-backend/static/images/kr/default2-120x120.png");
}
...
function resetLinkInput(data){
    if(data){
        ...
        $('#hot_name input[type=checkbox]:eq(0)').prop('checked',data.isNewWindow);
...
function addDefMediaHotSpotRes(data) {
    $(data).each(function (idx) {
        var type = this.type;
        var htmlStr = '<div class="col-md-1" style="padding-top:5px;padding-bottom:5px">'+
...
function toggleMediaTab(mediaType) {
    if (mediaType == 'image') {
        ...
        $('#imgUpload').prev().text("ファイル選択");
        $('#mediaTyPrompt').text("ファイル形式：音楽：MP3形式，画像：png、jpg形式。");
...
//        var sb = _U.getSubmit('/get_token.php', null, 'ajax', true);
    	var sb = _U.getSubmit('/myapp-backend/getToken', null, 'ajax', true);
...
function init() {
    ...
    var sb = _U.getSubmit("/myapp-backend/edit/pic", null, "ajax", false);
...
function initPage(data) {
    ...
    $("#cityInput").val(worksmain.lcposition);
...
function initWorkUseAndShare() {
   ...
   // var workslocation = "http://" + window.location.host + "/tour/" + worksmain.view_uuid;
	    var workslocation = "" + window.location.host + "/tour/" + worksmain.view_uuid;
...
function updateWorks() {
    ...
    worksMaindata.pro=$("#cityInput").val();
    ...
    sb.submit(function () {
        ...
        if (data.flag) {
            $.zui.messager.show('保存完了', {placement: 'center', type: 'success', time: '3000', icon: 'check'});
            if(imgsArr.length==0)
                window.location.href="/member/project";
        }else{
              $.zui.messager.show('保存失敗: '+data.msg, {placement: 'center', type: 'success', time: '3000', icon: 'check'});
        }
    });
}
...
function initPanoSetting(){
    ...
    $(".pano-setting-container button:eq(1)").click();
}
...
function initPanoSettingSceneChoose(){
    ...
    if(sceneGroup!=undefined && sceneGroup.sceneGroups && sceneGroup.sceneGroups.length > 0){
        ...
        var sceneGroupTitle = "初期シーン設定";
        var sceneGroupImageSrc = "/myapp-backend/static/images/skin1/vr-btn-scene.png";
        var imgType = "system";
...
function addSandTableImg(imgPath){
    var htmlStr = '<div class="sand-img-cont">'+
        '<img src="'+imgPath+'">'+
        '<span class="delete"><a href="javascript:void(0)">削除</a></span>'+
        '</div>';
    ...
    if(settingFlag){
        ...
        switch(idx){
            case 0:
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon" style="font-size:6px;">フォーカス</span><span class="group-change-icon" data-liindex="0" data-iconType="'+param.iconType+'" data-hotspotidx="0" style="font-size:6px;">アイコン</span>&nbsp;&nbsp;<span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="0" style="font-size:6px;">シーン&nbsp;&nbsp;</span><span class="group-delte" style="font-size:6px;">削除</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
                ...
                break;
            case 1:
                krpano.call('addLinkHotSpot("'+param.imgPath+'","'+ (param.name) +'",'+html_encode(param.hotspotTitle)+','+(param.ath)+','+(param.atv)+','+param.isDynamic+','+settingFlag+','+isBtnClicked(1)+','+param.link+','+param.isShowSpotName+','+param.isNewWindow+')');
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
        ...
        switch(idx){
            case 0:
                ...
                break;
            case 1:
                krpano.call('addLinkHotSpot("'+param.imgPath+'","'+ (param.name) +'",'+html_encode(param.hotspotTitle)+','+(param.ath)+','+(param.atv)+','+param.isDynamic+','+settingFlag+',true,'+param.link+','+param.isShowSpotName+','+param.isNewWindow+')');
                break;
            case 2:
				if(param.iconType=="polygon"){
                    createPolygonHotSpot(krpano,param);
...
function buildSceneGroupSetting(){
    ...
    //     return sceneGroup;
    // }
    if(sceneGroup && sceneGroup.sceneGroups && sceneGroup.sceneGroups.length > 0 && $('#groupbyul > li').length == 0){
        return sceneGroup;
    }
    sceneGroupObj = {};
