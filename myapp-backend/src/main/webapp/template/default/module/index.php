...
function get_index_recommend(){
	$sql = "select w.name,w.thumb_path,w.view_uuid,w.profile,w.browsing_num,p.avatar,w.praised_num,w.pk_user_main,u.nickname ".
	       "from ".$GLOBALS['Base']->table('worksmain')." as w".
	       " LEFT JOIN ".$GLOBALS['Base']->table('user_profile')." as p ON w.pk_user_main=p.pk_user_main ".
	       " LEFT JOIN ".$GLOBALS['Base']->table('user')." as u  ON w.pk_user_main=u.pk_user_main ".
	       " where w.recommend=1 order by w.sort asc, w.pk_works_main desc limit 20";
	$res = $GLOBALS['Db']->query($sql);
	return $res;
}
